...
class PlatformFFI {
  ...
  late InforiaConnectImpl _ffiBind;
  ...
  InforiaConnectImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ...
        : isLinux
            ? DynamicLibrary.open('librustdesk.so')
            : isWindows
                ? DynamicLibrary.open('libInforiaConnect.dll')
                :
    ...
    try {
      ...
      _ffiBind = InforiaConnectImpl(dylib);
  ...
  void _startListenEvent(InforiaConnectImpl InforiaConnectImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = InforiaConnectImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
