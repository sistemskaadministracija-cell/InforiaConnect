...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "InforiaConnect".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/InforiaConnect-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "InforiaConnect wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "InforiaConnect wants to uninstall the plugin",
    )
}
