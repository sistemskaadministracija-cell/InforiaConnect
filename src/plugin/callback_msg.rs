...
const MSG_TO_InforiaConnect_TARGET: &str = "InforiaConnect";
...
const MSG_TO_InforiaConnect_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
#[derive(Deserialize)]
pub struct MsgToInforiaConnect {
    pub r#type: String,
    pub data: Vec<u8>,
}
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_InforiaConnect_TARGET => handle_msg_to_InforiaConnect(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_InforiaConnect(id: String, content: *const c_void, len: usize) -> PluginReturn {
    let s = early_return_value!(
        std::str::from_utf8(unsafe { std::slice::from_raw_parts(content as _, len) }),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg string"
    );
    let msg_to_InforiaConnect = early_return_value!(
        serde_json::from_str::<MsgToInforiaConnect>(s),
        ERR_CALLBACK_INVALID_MSG,
        "parse msg '{}'",
        s
    );
    match &msg_to_InforiaConnect.r#type as &str {
        MSG_TO_InforiaConnect_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_InforiaConnect),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_InforiaConnect_TARGET
            ),
        ),
    }
}

fn request_plugin_sign(id: String, msg_to_InforiaConnect: MsgToInforiaConnect) -> PluginReturn {
    let signature_data = early_return_value!(
        std::str::from_utf8(&msg_to_InforiaConnect.data),
        ERR_CALLBACK_INVALID_MSG,
