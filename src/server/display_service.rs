...
// https://github.com/InforiaConnect/InforiaConnect/discussions/6042, avoiding dbus call
...
// https://github.com/InforiaConnect/InforiaConnect/pull/8537
...
pub fn check_displays_changed() -> ResultType<()> {
    #[cfg(target_os = "linux")]
    {
        ...
        // block, or even crash here, https://github.com/InforiaConnect/InforiaConnect/blob/0bb4d43e9ea9d9dfb9c46c8d27d1a97cd0ad6bea/libs/scrap/src/wayland/pipewire.rs#L235
        if !is_x11() {
            return Ok(());
        }
    }
...
pub(super) fn get_original_resolution(
    ...
    #[cfg(windows)]
    let is_InforiaConnect_virtual_display =
        crate::virtual_display_manager::InforiaConnect_idd::is_virtual_display(&display_name);
    #[cfg(not(windows))]
    let is_InforiaConnect_virtual_display = false;
    Some(if is_InforiaConnect_virtual_display {
        Resolution {
            width: 0,
            height: 0,
            ..Default::default()
