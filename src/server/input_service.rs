...
// https://github.com/InforiaConnect/InforiaConnect/issues/9729
...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // It looks like https://github.com/InforiaConnect/InforiaConnect/issues/9729#issuecomment-2432306822
            // 1. Press "Command" key in InforiaConnect
            // 2. Move the physical mouse
            // 3. Press "V" key in InforiaConnect
            ...
            // we consider only InforiaConnect operates the controlled side.
            //
...
#[cfg(target_os = "macos")]
fn key_sleep() {
    // https://www.reddit.com/r/InforiaConnect/comments/1kn1w5x/typing_lags_when_connecting_to_macos_clients/
    //
    ...
    // Though `/Applications/InforiaConnect.app/Contents/MacOS/InforiaConnect --server` in terminal is ok.
    let now = Instant::now();
...
fn get_modifier_state(key: Key, en: &mut Enigo) -> bool {
    // https://github.com/InforiaConnect/InforiaConnect/issues/332
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run InforiaConnect from cmd
        std::thread::spawn(|| {
...
// https://github.com/InforiaConnect/InforiaConnect/issues/3928#issuecomment-1496936687
// https://github.com/InforiaConnect/InforiaConnect/issues/3928#issuecomment-1500415822
// https://github.com/InforiaConnect/InforiaConnect/issues/3928#issuecomment-1500773473
#[cfg(any(target_os = "windows", target_os = "linux"))]
