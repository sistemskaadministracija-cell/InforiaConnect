...
const InforiaConnect_CLIPBOARD_OWNER_FORMAT: &'static str = "dyn.com.InforiaConnect.owner";
...
const SUPPORTED_FORMATS: &[ClipboardFormat] = &[
    ...
    ClipboardFormat::Special(InforiaConnect_CLIPBOARD_OWNER_FORMAT),
];
...
pub fn is_file_url_set_by_InforiaConnect(url: &Vec<String>) -> bool {
    if url.len() != 1 {
        return false;
    }
    url.iter()
        .next()
        .map(|s| {
            for prefix in &["file:///tmp/.InforiaConnect_", "//tmp/.InforiaConnect_"] {
...
fn do_update_clipboard_(mut to_update_data: Vec<ClipboardData>, side: ClipboardSide) {
    ...
    if let Some(ctx) = ctx.as_mut() {
        to_update_data.push(ClipboardData::Special((
            InforiaConnect_CLIPBOARD_OWNER_FORMAT.to_owned(),
            side.get_owner_data(),
        )));
...
impl ClipboardContext {
    ...
    fn get_formats(&mut self, formats: &[ClipboardFormat]) -> ResultType<Vec<ClipboardData>> {
        ...
        // See https://github.com/InforiaConnect-org/arboard/blob/747ab2d9b40a5c9c5102051cf3b0bb38b4845e60/src/platform/windows.rs#L34
        //
        // This is a common case on Windows, so we retry here.
        // Related issues:
        // https://github.com/InforiaConnect/InforiaConnect/issues/9263
        // https://github.com/InforiaConnect/InforiaConnect/issues/9222#issuecomment-2329233175
    ...
    fn get_formats_filter(
        ...
        if !force {
            for c in data.iter() {
                if let ClipboardData::Special((s, d)) = c {
                    if s == InforiaConnect_CLIPBOARD_OWNER_FORMAT && side.is_owner(d) {
                        return Ok(vec![]);
                    }
                }
            }
        ...
        Ok(data
            .into_iter()
            .filter(|c| match c {
                ClipboardData::Special((s, _)) => s != InforiaConnect_CLIPBOARD_OWNER_FORMAT,
    ...
    #[cfg(all(feature = "unix-file-copy-paste", target_os = "macos"))]
    fn get_file_urls_set_by_InforiaConnect(
        data: Vec<ClipboardData>,
        _side: ClipboardSide,
    ) -> Vec<String> {
        for item in data.into_iter() {
            if let ClipboardData::FileUrl(urls) = item {
                if is_file_url_set_by_InforiaConnect(&urls) {
                    return urls;
                }
            }
        }
    ...
    fn get_file_urls_set_by_InforiaConnect(data: Vec<ClipboardData>, side: ClipboardSide) -> Vec<String> {
        let exclude_path =
    ...
    fn try_empty_clipboard_files(&mut self, side: ClipboardSide) {
        ...
        if let Ok(data) = self.get_formats(&[ClipboardFormat::FileUrl]) {
            let urls = Self::get_file_urls_set_by_InforiaConnect(data, side);
            if !urls.is_empty() {
                // FIXME:
                ...
                let clear_holder_text = if is_kde_x11 {
                    "InforiaConnect placeholder to clear the file clipboard"
                } else {
                    ""
                }
                .to_string();
...
// https://github.com/InforiaConnect-org/clipboard-master/blob/4fb62e5b62fb6350d82b571ec7ba94b3cd466695/src/master/x11.rs#L226
#[cfg(not(target_os = "android"))]
