    ...
    loop {
        ...
        if !sids.contains(&session_id) || !is_share_rdp() {
            ...
            if session_id != current_active_session {
                ...
                // https://github.com/InforiaConnect/InforiaConnect/discussions/10039
...
pub fn portable_service_logon_helper_paths() -> Option<(PathBuf, PathBuf)> {
    ...
    let dir = user_dir
        ...
        .join("AppData")
        .join("Local")
        .join("InforiaConnect-sciter");
    let dst = dir.join("InforiaConnect.exe");
    Some((dir, dst))
}
...
fn get_after_install(
    ...
    // https://github.com/InforiaConnect/InforiaConnect/commit/f4bdfb6936ae4804fc8ab1cf560db192622ad01a
...
copy /Y \"{tmp_path}\\Uninstall {app_name}.lnk\" \"{start_menu}\\\"
    ...
    // Note: without if exist, the bat may exit in advance on some Windows7 https://github.com/InforiaConnect/InforiaConnect/issues/895
    let dels = format!(
        "
...
fn run_cmds(cmds: String, show: bool, tip: &str) -> ResultType<()> {
    ...
    // https://github.com/InforiaConnect/InforiaConnect/issues/6786#issuecomment-1879655410
...
pub fn get_custom_client_staging_dir() -> PathBuf {
    get_public_base_dir()
        .join("InforiaConnect")
        .join("InforiaConnectCustomClientStaging")
}
...
/// - Deleting directories under a public location (e.g., C:\\ProgramData\\InforiaConnect) is
...
pub fn bootstrap() -> bool {
    ...
    #[cfg(not(debug_assertions))]
    {
        // This function will cause `'sciter.dll' was not found neither in PATH nor near the current executable.` when debugging InforiaConnect.
...
pub fn create_shortcut(id: &str) -> ResultType<()> {
    ...
    // https://github.com/InforiaConnect/InforiaConnect/issues/13735
    // Replace ':' with '_' for filename since ':' is not allowed in Windows filenames
    // https://github.com/InforiaConnect/hbb_common/blob/8b0e25867375ba9e6bff548acf44fe6d6ffa7c0e/src/config.rs#L1384
...
pub fn get_unicode_from_vk(vk: u32) -> Option<u16> {
    ...
    let len = unsafe {
        ...
        // refs: https://github.com/InforiaConnect-org/rdev/blob/25a99ce71ab42843ad253dd51e6a35e83e87a8a4/src/windows/keyboard.rs#L115
        let press_state = 129;
...
reg add {subkey} /f /v EstimatedSize /t REG_DWORD /d {size}
    ...
    // 1. There may be some other processes like `InforiaConnect --connect` are running.
    ...
    for entry in entries {
        if let Ok(entry) = entry {
            ...
            if let Some(file_name) = path.file_name().and_then(|n| n.to_str()) {
                // Match files like InforiaConnect-*.msi or InforiaConnect-*.exe
                if file_name.starts_with("InforiaConnect-")
...
pub fn message_box(text: &str) {
    ...
    let text = text
        ...
        .collect::<Vec<u16>>();
    let caption = "InforiaConnect Output"
        .encode_utf16()
        .chain(std::iter::once(0))
        .collect::<Vec<u16>>();
...
pub fn try_kill_InforiaConnect_main_window_process() -> ResultType<()> {
    // Kill InforiaConnect.exe without extra arg, should only be called by --server
    ...
    bail!("failed to find InforiaConnect main window process");
}
...
pub fn send_raw_data_to_printer(printer_name: Option<String>, data: Vec<u8>) -> ResultType<()> {
    ...
    unsafe {
        ...
        if res != 0 {
            bail!("Failed to send data to the printer, see logs in C:\\Windows\\temp\\test_InforiaConnect.log for more details.");
        } else {
...
fn get_pids_with_args_from_wmic_output<S2: AsRef<str>>(
    ...
    // ProcessId=34668
    //
    // CommandLine="C:\Program Files\InforiaConnect\InforiaConnect.exe" --tray
    // ProcessId=13728
    //
    // CommandLine="C:\Program Files\InforiaConnect\InforiaConnect.exe"
    // ProcessId=10136
    let mut pids = Vec::new();
