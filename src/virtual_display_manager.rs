...
//  https://github.com/InforiaConnect-org/InforiaConnectIddDriver/blob/b370aad3f50028b039aad211df60c8051c4a64d6/InforiaConnectIddDriver/InforiaConnectIddDriver.inf#LL73C1-L73C40
pub const InforiaConnect_IDD_DEVICE_STRING: &'static str = "InforiaConnectIddDriver Device\0";
pub const AMYUNI_IDD_DEVICE_STRING: &'static str = "USB Mobile Monitor Virtual Display\0";

const IDD_IMPL: &str = IDD_IMPL_AMYUNI;
const IDD_IMPL_InforiaConnect: &str = "InforiaConnect_idd";
...
pub fn get_cur_device_string() -> &'static str {
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_IDD_DEVICE_STRING,
...
pub fn plug_in_headless() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_idd::plug_in_headless(),
...
pub fn get_platform_additions() -> serde_json::Map<String, serde_json::Value> {
    ...
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => {
            let virtual_displays = InforiaConnect_idd::get_virtual_displays();
            if !virtual_displays.is_empty() {
                map.insert(
                    "InforiaConnect_virtual_displays".into(),
...
pub fn plug_in_monitor(idx: u32, modes: Vec<virtual_display::MonitorMode>) -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_idd::plug_in_index_modes(idx, modes),
    ...
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => {
            let indices = if index == IDD_PLUG_OUT_ALL_INDEX {
                InforiaConnect_idd::get_virtual_displays()
            } else {
                vec![index as _]
            };
            InforiaConnect_idd::plug_out_peer_request(&indices)
        }
...
pub fn plug_in_peer_request(modes: Vec<Vec<virtual_display::MonitorMode>>) -> ResultType<Vec<u32>> {
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_idd::plug_in_peer_request(modes),
        IDD_IMPL_AMYUNI => {
...
pub fn plug_out_monitor_indices(
    ...
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_idd::plug_out_peer_request(indices),
        IDD_IMPL_AMYUNI => {
...
pub fn reset_all() -> ResultType<()> {
    match IDD_IMPL {
        IDD_IMPL_InforiaConnect => InforiaConnect_idd::reset_all(),
        IDD_IMPL_AMYUNI => amyuni_idd::reset_all(),
        _ => bail!("Unsupported virtual display implementation."),
    }
}

pub mod InforiaConnect_idd {
    use super::windows;
    ...
    fn get_device_names() -> Vec<String> {
        windows::get_device_names(Some(super::InforiaConnect_IDD_DEVICE_STRING))
    }
...
pub mod amyuni_idd {
    ...
    // 2. InforiaConnect may crash and restart, but the virtual displays are kept.
    //
    ...
    //             if there're no virtual displays managed by InforiaConnect.
    pub fn plug_out_monitor(index: i32, force_all: bool, force_one: bool) -> ResultType<()> {
        ...
        //
        // 1. InforiaConnect plug in 2 virtual displays. (InforiaConnect)
        // 2. Other process plug out all virtual displays. (User manually)
        // 3. Other process plug in 1 virtual display. (User manually)
        // 4. InforiaConnect plug out all virtual displays in this call. (InforiaConnect disconnect)
        //
        // This is not a normal scenario, InforiaConnect will plug out virtual display unexpectedly.
