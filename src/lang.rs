...
pub fn translate_locale(name: String, locale: &str) -> String {
    ...
    let replace = |s: &&str| {
        ...
        if !crate::is_InforiaConnect() {
            if s.contains("InforiaConnect")
                && !name.starts_with("upgrade_InforiaConnect_server_pro")
                && name != "powered_by_me"
            {
                let app_name = crate::get_app_name();
                if !app_name.contains("InforiaConnect") {
                    s = s.replace("InforiaConnect", &app_name);
                } else {
                    // https://github.com/InforiaConnect/InforiaConnect-server-pro/issues/845
                    // If app_name contains "InforiaConnect" (e.g., "InforiaConnect-Admin"), we need to avoid
                    // replacing "InforiaConnect" within the already-substituted app_name, which would
                    // cause duplication like "InforiaConnect-Admin" -> "InforiaConnect-Admin-Admin".
                    //
